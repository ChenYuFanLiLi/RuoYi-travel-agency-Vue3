  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
                  ...
                  <el-form-item label="合计转出" prop="costTotalTransfer">
                    <el-input
                        v-model="queryParams.costTotalTransfer"
                        placeholder="请输入合计转出"
                        clearable
                        @keyup.enter="handleQuery"
                    />
                  ...
                  <el-form-item label="导游垫款" prop="costGuideAdvance">
                    <el-input
                        v-model="queryParams.costGuideAdvance"
                        placeholder="请输入导游垫款"
                        clearable
                        @keyup.enter="handleQuery"
                    />
                  ...
                  <el-form-item label="报账明细" prop="costReimbursementDetail">
                    <el-input
                        v-model="queryParams.costReimbursementDetail"
                        placeholder="请输入报账明细"
                        clearable
                        @keyup.enter="handleQuery"
                    />
    ...
    <el-table v-loading="loading" :data="accountingList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              ...
              <el-table-column label="操作计划" align="center" prop="operationPlanId" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['travel:accounting:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['travel:accounting:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="accountingRef" :model="form" :rules="rules" label-width="80px">
                        ...
                        <el-form-item label="团队编号" prop="teamNumber">
                          <el-input v-model="form.teamNumber" placeholder="请输入团队编号" />
                        </el-form-item>
                        <el-form-item label="合计转出" prop="costTotalTransfer">
...
<script setup name="Accounting">
  import { listAccounting, getAccounting, delAccounting, addAccounting, updateAccounting } from "@/api/travel/accounting";
  ...
  function handleAdd() {
    reset();
    open.value = true;
    title.value = "添加成本核算，用于记录团队成本核算信息";
  }
  ...
  function handleUpdate(row) {
    ...
    getAccounting(_id).then(response => {
      ...
      open.value = true;
      title.value = "修改成本核算，用于记录团队成本核算信息";
    });
  }
  ...
  function handleDelete(row) {
    ...
    proxy.$modal.confirm('是否确认删除成本核算，用于记录团队成本核算信息编号为"' + _ids + '"的数据项？').then(function() {
